...
describe("Industrial Drone Flow", function () {
  ...
  const manager = await Manager.deploy(await registry.getAddress());
  await manager.waitForDeployment();

    const Logger = await ethers.getContractFactory("DataLogger");
  const logger = await Logger.deploy(await manager.getAddress());
  ...
  const dataHash = ethers.keccak256(ethers.toUtf8Bytes("result-ok"));
